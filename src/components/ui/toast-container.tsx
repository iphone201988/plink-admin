...
import { ToastPosition, Toast } from "@/types";
...
export function ToastContainer() {
  ...
  useEffect(() => {
    // Check if there's a toast with a custom position
    const customPositionToast = toasts.find(toast => toast.position);
    if (customPositionToast && customPositionToast.position) {
  ...
  return (
    <div className={`toast-container fixed flex flex-col gap-2 z-50 ${getPositionClasses()}`}>
      <AnimatePresence>
        {toasts.map((toast) => (
          <motion.div
            key={toast.id}
            ...
            className="toast-notification bg-white dark:bg-gray-800 shadow-lg rounded-lg p-4 flex items-start space-x-3 w-72 transform transition-transform duration-300"
          >
